...
The spawned Codex workers run in Codex's non-interactive bypass mode and inject a Playwright-only MCP override, so those child sessions do not inherit unrelated browser plugins such as Camoufox from the global Codex config.
